...
void usage(char *progname)
{
    ...
    printf("     *              Return all services registered\n");
        ...
        if ((argv[i][0] == '-') || (argv[i][0] == '/')) 
        {
            switch (tolower(argv[i][1])) 
            {
                ...
                case 's':        // Server, register as the given service
                    ...
                    lstrcpy(szServerName, argv[++i]);
                    break;
                case 't':        // Unique ID to register service under
                    if (i+1 >= argc)
                        usage(argv[0]);
                    dwUniqueId = atoi(argv[++i]);
                    break;
                default:
                    usage(argv[0]);
                    return;
...
BOOL InstallServiceClass(GUID *svcguid, WSANAMESPACE_INFO *nsinfo, int nscount)
{
    ...
    //
    wsprintf(szServiceClassName, TEXT("Service Class/TCP Port: %05d"), dwUniqueId);
